...
Public Class RDLCPrinterPrintNew : Implements IDisposable
    ...
    Public Sub New(ByVal SampleID As String, ByVal Section As String, ByVal SubSection As String, ByVal PrintStatus As String, ByVal DefaultPrinterName As String)
        ...
        Dim SQL = "SELECT * FROM `tmpWorklist` WHERE `main_id` = '" & SampleID & "' AND testtype = '" & Section & "' AND sub_section = '" & SubSection & "'"
        ...
        'Result Table
        Connect()
        Dim result_SQL = "SELECT * FROM `tmpresult` WHERE `sample_id` = '" & SampleID & "' AND `print_status` = '1' AND section = '" & Section & "' AND sub_section = '" & SubSection & "' ORDER BY `order_no`"
