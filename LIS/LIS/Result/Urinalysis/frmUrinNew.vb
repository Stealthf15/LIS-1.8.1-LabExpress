...
Public Class frmUrinNew
    ...
    Public Sub LoadTest()
        ...
        Try
            ...
            Dim SQL As String = "SELECT `universal_id` AS TestName, `flag` AS Flag, `measurement` AS SI, `units` as Unit,
                ...
                `ref_conv` AS ConvRefRange, `his_mainid` AS Remarks, `instrument` AS Instrument, `test_code` AS TestCode, `id` AS ID, 
    ...
    '        rs.Parameters.AddWithValue("@age", txtAge.Text & " " & cboSex.Text)
    ...
    '        rs.Parameters.Add("@date", MySql.Data.MySqlClient.MySqlDbType.Date).Value = dtReceived.Value.ToString
    '        rs.Parameters.AddWithValue("@date_released", Now)
    '        rs.Parameters.AddWithValue("@medtech", cboMedTech.Text)
    '        rs.Parameters.AddWithValue("@section", Section)
    '        rs.Parameters.AddWithValue("@time", Now.TimeOfDay)
    '        rs.Parameters.AddWithValue("@lab_no", txtAccession.Text)
    '        rs.Parameters.AddWithValue("@Shift", getShift(Now.TimeOfDay)) 'Get Shift

    '        Connect()
    '        rs.Connection = conn
    '        rs.CommandType = CommandType.Text
    '        rs.CommandText = "UPDATE `worksheet` SET " _
    '            & "`dt_released` = @date_released," _
    '            & "`sample_id` = @sample_id," _
    '            & "`patient_id` = @patient_id," _
    '            & "`patient_name` = @name," _
    '            & "`age` = @age," _
    '            & "`bdate` = @bdate," _
    '            & "`requestor` = @physician," _
    '            & "`dept` = @room," _
    '            & "`medtech` = @medtech," _
    ...
    '            For x = 0 To dtResult.Rows.Count - 1 Step 1
    '                Connect()
    '                rs.Connection = conn
    '                rs.CommandType = CommandType.Text
    '                rs.CommandText = "UPDATE `worksheet` SET `" & dtResult.Rows(x).Cells(10).Value & "` = '" & dtResult.Rows(x).Cells(3).Value & "'," _
    '                    & "`instrument` = '" & dtResult.Rows(x).Cells(9).Value & "'" _
    '                    & " WHERE `main_id` = @mainID"
    '                rs.ExecuteNonQuery()
    '                Disconnect()
    '            Next
    ...
    '                rs.Connection = conn
    '                rs.CommandType = CommandType.Text
    '                rs.CommandText = "UPDATE `worksheet` SET `" & dtResult.Rows(x).Cells(10).Value & "` = '" & dtResult.Rows(x).Cells(6).Value & "'," _
    '                    & "`instrument` = '" & dtResult.Rows(x).Cells(9).Value & "'" _
    '                    & " WHERE `main_id` = @mainID"
    '                rs.ExecuteNonQuery()
    '                Disconnect()
